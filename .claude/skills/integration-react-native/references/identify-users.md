...
2.  Have your backend issue an authenticated or cryptographically signed, short-lived, one-time identity-link token. Bind the token to the exact `ph_distinct_id`, the intended app or audience, and a unique nonce, then add both the distinct ID and token to the deep link alongside properties like UTM parameters. Never trust an unsigned `ph_distinct_id` on its own.
3.  When the user is redirected to the app, send the token and `ph_distinct_id` to your backend for validation. The backend must verify the authentication or signature, expiry, audience, and distinct-ID binding, then atomically consume the nonce. Reject invalid or previously consumed link data before calling `posthog.alias()` or `posthog.identify()`, and handle the following cases only after validation succeeds:
...
Identity-link validation only determines whether the incoming PostHog distinct ID is safe to associate. It must not sign a user into an application account, grant permissions, or otherwise change your application's authentication or authorization behavior.
...
```swift
import PostHog
protocol IdentityLinkVerifier {
    // The backend validates the token and atomically consumes its one-time nonce.
    func consume(token: String, webDistinctId: String) async -> Bool
}
final class DeepLinkIdentityManager {
    private let identityLinkVerifier: IdentityLinkVerifier
    init(identityLinkVerifier: IdentityLinkVerifier) {
        self.identityLinkVerifier = identityLinkVerifier
    }
    // MARK: - Deep Link Received
    func handleDeepLink(_ url: URL, isAuthenticatedOnMobile: Bool) async {
        let queryItems = URLComponents(url: url, resolvingAgainstBaseURL: true)?.queryItems
        guard let webDistinctId = queryItems?
            .first(where: { $0.name == "ph_distinct_id" })?.value,
            let identityLinkToken = queryItems?
            .first(where: { $0.name == "ph_identity_link_token" })?.value,
            await identityLinkVerifier.consume(
                token: identityLinkToken,
                webDistinctId: webDistinctId
            ) else {
            // Reject missing, unverified, expired, or previously used link data.
            return
        }
...
interface IdentityLinkVerifier {
    // The backend validates the token and atomically consumes its one-time nonce.
    suspend fun consume(token: String, webDistinctId: String): Boolean
}
class DeepLinkIdentityManager(
    private val identityLinkVerifier: IdentityLinkVerifier,
) {
    // Deep Link Received
    suspend fun handleDeepLink(uri: Uri, isAuthenticatedOnMobile: Boolean) {
        val webDistinctId = uri.getQueryParameter("ph_distinct_id") ?: return
        val identityLinkToken = uri.getQueryParameter("ph_identity_link_token") ?: return
        if (!identityLinkVerifier.consume(identityLinkToken, webDistinctId)) {
            // Reject unverified, expired, or previously used link data.
            return
        }
        if (isAuthenticatedOnMobile) {
            // The mobile app already knows the current user.
